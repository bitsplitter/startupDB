...
const db = function (options: DBOptions) {
    return async function (req: Req, res: Res, next: NextFunction) {
        ...
        req.startupDB.collection = requestUrl.split('?')[0].substring(1)
        ...
        req.startupDB.pullOplog = async function (collection: string, query) {
            ...
            await processOplog(collection, db, startupDB[collectionId].nextOpLogId - 1, function (operation: Operation, length: number) {
                startupDB[collectionId].nextOpLogId = operation.opLogId + 1
                applyCRUDoperation(operation, db, length)
            })
        }
